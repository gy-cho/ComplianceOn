...
-- 준법 이미지 목록 테이블
CREATE TABLE TB_COMP_IMG_POOL (
  IMG_ID SERIAL PRIMARY KEY,
  IMG_FLNM varchar(300) NOT NULL,
  IMG_TYPE varchar(20),
  DEL_YN varchar(20) ,
  REG_EMP_NO varchar(10) ,
  REG_DTM timestamp DEFAULT now(),
...
COMMENT ON TABLE TB_COMP_IMG_POOL IS '준법 이미지 목록 테이블';
COMMENT ON COLUMN TB_COMP_IMG_POOL.IMG_ID IS '이미지아이디';
COMMENT ON COLUMN TB_COMP_IMG_POOL.IMG_FLNM IS '이미지파일명';
COMMENT ON COLUMN TB_COMP_IMG_POOL.IMG_TYPE IS '이미지종류';
COMMENT ON COLUMN TB_COMP_IMG_POOL.DEL_YN IS '삭제여부';
COMMENT ON COLUMN TB_COMP_IMG_POOL.REG_EMP_NO IS '최초등록자';
COMMENT ON COLUMN TB_COMP_IMG_POOL.REG_DTM IS '최초등록일시';
COMMENT ON COLUMN TB_COMP_IMG_POOL.CHG_EMP_NO IS '최종수정자';
COMMENT ON COLUMN TB_COMP_IMG_POOL.CHG_DTM IS '최종수정일시';




-- TASK 적용일별직원답변여부
CREATE TABLE TB_COMP_TASK_APP_DT_EMP_ANS (
  TASK_ID int,
  APP_SEQ int,
  EMP_NO varchar(10) NOT NULL,
  EMP_MAIN_ANS_YN varchar(1),
  EMP_ANS_AGR_YN varchar(1),
  ANS_DT timestamp NOT NULL DEFAULT now(),
  DEL_YN varchar(20) ,
  REG_EMP_NO varchar(10) ,
  REG_DTM timestamp DEFAULT now(),
  ...
  PRIMARY KEY (TASK_ID, APP_SEQ, EMP_NO)
);

COMMENT ON TABLE TB_COMP_TASK_APP_DT_EMP_ANS IS '적용일별직원답변여부';
COMMENT ON COLUMN TB_COMP_TASK_APP_DT_EMP_ANS.TASK_ID IS '타스크아이디';
COMMENT ON COLUMN TB_COMP_TASK_APP_DT_EMP_ANS.APP_SEQ IS '순번';
COMMENT ON COLUMN TB_COMP_TASK_APP_DT_EMP_ANS.EMP_NO IS '직원번호';
COMMENT ON COLUMN TB_COMP_TASK_APP_DT_EMP_ANS.EMP_MAIN_ANS_YN IS '직원별답변여부';
COMMENT ON COLUMN TB_COMP_TASK_APP_DT_EMP_ANS.EMP_ANS_AGR_YN IS '직원별동의여부';
COMMENT ON COLUMN TB_COMP_TASK_APP_DT_EMP_ANS.ANS_DT IS '답변일시';
COMMENT ON COLUMN TB_COMP_TASK_APP_DT_EMP_ANS.DEL_YN IS '삭제여부';
COMMENT ON COLUMN TB_COMP_TASK_APP_DT_EMP_ANS.REG_EMP_NO IS '최초등록자';
COMMENT ON COLUMN TB_COMP_TASK_APP_DT_EMP_ANS.REG_DTM IS '최초등록일시';
COMMENT ON COLUMN TB_COMP_TASK_APP_DT_EMP_ANS.CHG_EMP_NO IS '최종수정자';
COMMENT ON COLUMN TB_COMP_TASK_APP_DT_EMP_ANS.CHG_DTM IS '최종수정일시';