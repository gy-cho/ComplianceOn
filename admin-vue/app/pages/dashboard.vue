<template>
  <div class="page-wrapper">
    ...
    <div class="page-header">
      <span class="page-title">현황조회</span>
      <button class="btn-icon" title="새로고침" @click="refresh">↻</button>
    </div>
      ...
      <div class="row" style="gap:12px">
        <div style="flex:2.5">
          <select
            ...
            @change="onTaskChange"
          >
            <option v-if="taskList.length === 0" :value="null">등록된 준법 항목이 없습니다.</option>
        ...
        <div style="flex:1">
          <template v-if="appSeqOptions.length > 0">
            <select v-model="selectedAppSeq" class="form-select" @change="onSeqChange">
              <option v-for="opt in appSeqOptions" :key="opt.seq" :value="opt.seq">
                {{ opt.label }}
              </option>
            </select>
          </template>
          <div v-else class="info-box" style="padding:8px 12px">할당된 적용일 없음</div>
        </div>
      </div>
    </div>
    ...
    <template v-else-if="selectedTaskId && selectedAppSeq !== null && answers.length > 0">
      <!-- Stats Cards -->
      <div class="stats-row">
        <div class="metric-card">
          ...
          <button class="btn btn-secondary btn-sm mt-8" @click="statusFilter = '전체'">전체 대상자 보기</button>
        </div>
        <div class="metric-card">
          ...
          <button class="btn btn-secondary btn-sm mt-8" @click="statusFilter = '완료'">답변 완료자 보기</button>
        </div>
        <div class="metric-card">
          ...
          <button class="btn btn-secondary btn-sm mt-8" @click="statusFilter = '미완료'">미답변자 보기</button>
        </div>
      </div>
    </template>

    <!-- Card 2: Table -->
    <div v-if="selectedTaskId && selectedAppSeq !== null && !loading" class="card">
          ...
          <table class="data-table">
            ...
            <tbody>
              ...
              >
                ...
                <td>{{ row.task_nm }}({{ row.app_seq }}회차) - {{ row.task_app_dt }}</td>
                <td>{{ row.ip || '-' }}</td>
                <td>
                  <span :class="isAnswered(row) ? 'badge badge-success' : 'badge badge-error'">
                    {{ isAnswered(row) ? '완료' : '미완료' }}
                  </span>
                </td>
                <td>
                  <span v-if="isAnswered(row)" :class="isAgreed(row) ? 'badge badge-success' : 'badge badge-error'">
                    {{ isAgreed(row) ? '정상' : '비정상' }}
                  </span>
...
const pendingCount = computed(() => answers.value.filter((r) => !isAnswered(r)).length)
const todayDone = computed(() =>
  answers.value.filter((r) => isAnswered(r) && (r.ans_dt ?? '').includes(today)).length
)

const filteredAnswers = computed(() => {
  let data = [...answers.value]
  if (statusFilter.value === '완료') data = data.filter(isAnswered)
  else if (statusFilter.value === '미완료') data = data.filter((r) => !isAnswered(r))
  if (searchText.value) {
...
async function onTaskChange() {
  selectedAppSeq.value = appSeqOptions.value.length > 0 ? appSeqOptions.value[0].seq : null
  answers.value = []
