  ...
  <div class="page-wrapper">
    ...
    <template v-if="mode === 'list'">
      <div class="page-header">
        <span class="page-title">준법 TASK 목록</span>
      ...
      <div class="card">
          ...
          <table class="data-table">
            ...
            <tbody>
              ...
              >
                <td>{{ t.task_nm }}</td>
                <td>
                  <span class="badge badge-yellow">{{ typeLabel(t.task_type) }}</span>
                </td>
                <td>
                  <span :class="t.pbls_yn === 'Y' ? 'badge badge-success' : 'badge badge-gray'">
                    {{ t.pbls_yn }}
                  </span>
                </td>
              </tr>
        ...
        <div v-else class="info-box">등록된 준법 관리 TASK 항목이 존재하지 않습니다.</div>
      </div>
    </template>
    ...
    <template v-else-if="mode === 'create'">
      ...
      <div class="card">
        ...
        <div class="form-group">
          ...
          <input v-model="form.task_nm" class="form-input" placeholder="예: 2026년 하반기 정보보안 서약 관리" />
        </div>
      ...
      <div class="card">
        ...
        <div v-else class="info-box mt-8">선택된 적용일이 없습니다. 날짜를 추가해 주세요.</div>
      </div>
        ...
        <template v-else-if="form.task_type === 'SELF_CHECK'">
          ...
          <div class="question-select-wrapper">
            <div class="question-select-search">
              <input v-model="qSearch" class="form-input" placeholder="문항 검색..." style="font-size:13px; padding:6px 10px" />
            </div>
            <label
            ...
            >
              <input type="checkbox" :value="q.qstn_cd" v-model="form.selected_qstn_cds" />
              <span class="question-option-text">
                [{{ q.qstn_cd }}] {{ q.qstn_nm }} - {{ q.qstn_cn }}
              </span>
            </label>
            <div v-if="filteredQuestions.length === 0" class="question-option text-muted">검색 결과 없음</div>
          </div>
          <p class="text-muted mt-8">선택됨: {{ form.selected_qstn_cds.length }}개</p>
        </template>
      </div>

      <!-- Action buttons -->
      <div class="row" style="gap:10px; max-width:320px">
        <button class="btn btn-primary col-1" @click="handleCreate">등록</button>
        <button class="btn btn-secondary col-1" @click="mode = 'list'">취소</button>
      </div>
    </template>
    ...
    <template v-else-if="mode === 'detail' && selectedTask">
      ...
      <div class="card">
        ...
        <div class="row" style="gap:16px">
          <div class="col-1">
            ...
            <input :value="typeLabel(selectedTask.task_type)" class="form-input" disabled />
          </div>
          <div class="col-1">
      ...
      <div class="card">
        ...
        <div v-else class="info-box mt-8">할당된 적용일이 없습니다. 날짜를 할당해 주세요.</div>
      </div>
      ...
      <div class="card">
        ...
        <template v-else-if="selectedTask.task_type === 'SELF_CHECK'">
          <p class="text-muted mb-8">본 준법 자가점검 TASK에 포함되어 있는 질문 문항 목록입니다.</p>
      ...
      <!-- Action buttons -->
      <div class="row" style="gap:10px; max-width:420px">
        <button class="btn btn-primary col-1" @click="handleUpdate">저장</button>
        <button class="btn btn-danger col-1" @click="handleDelete">삭제</button>
        <button class="btn btn-secondary col-1" @click="mode = 'list'">목록으로</button>
      </div>
    </template>
...
function typeLabel(type: string) {
  return type === 'ETHICS' ? '윤리강령' : type === 'SELF_CHECK' ? '자가점검' : type
}
...
async function openCreate() {
  form.value = { task_nm: '', task_type: 'ETHICS', pbls_yn: 'Y', img_flnm: null, selected_qstn_cds: [] }
  tempDates.value = []
  pickedDate.value = todayStr()
  usedDates.value = await fetchAllUsedDates()
  imageList.value = await fetchTaskImages()
  if (imageList.value.length > 0) form.value.img_flnm = imageList.value[0].img_flnm
...
async function openDetail(task: any) {
  ...
  if (task.task_type === 'SELF_CHECK') {
    ...
    questionPool.value = await fetchQuestionPool()
    detailQuestions.value = questionPool.value.filter((q) => mappedCds.includes(q.qstn_cd))
  }
  mode.value = 'detail'
}
...
async function handleCreate() {
  if (!form.value.task_nm.trim()) { showToast('error', 'TASK 명을 입력해 주세요.'); return }
  if (tempDates.value.length === 0) { showToast('error', '최소 1개 이상의 적용 날짜를 추가하셔야 합니다.'); return }
  if (form.value.task_type === 'ETHICS' && !form.value.img_flnm) { showToast('error', '서약 이미지를 선택해야 합니다.'); return }
  if (form.value.task_type === 'SELF_CHECK' && form.value.selected_qstn_cds.length === 0) {
    showToast('error', '최소 1개 이상의 질문 문항을 매핑해야 합니다.'); return
  }
  const payload = {
    ...
    selected_qstn_cds: form.value.task_type === 'SELF_CHECK' ? form.value.selected_qstn_cds : [],
...
async function handleUpdate() {
  ...
  const payload = {
    ...
    task_cn: null,
    pbls_yn: form.value.pbls_yn,
    img_flnm: selectedTask.value.task_type === 'ETHICS' ? form.value.img_flnm : null,
    app_dates: tempDates.value,
    emp_no: 'admin',
  }
