...
def show_add_employee_dialog():
    new_emp_name = st.text_input("이름", placeholder="이름을 입력하세요", label_visibility="collapsed")
    new_emp_id = st.text_input("사번", placeholder="사번을 입력하세요", label_visibility="collapsed")
    ...
    with col2:
        if st.button("저장", type="primary", use_container_width=True):
            if new_emp_name and new_emp_id:
                code, res = add_new_user(new_emp_id, new_emp_name)
...
def show_dashboard_page():
    ...
    if not df.empty:
        ...
        # --------------------------------------------------------------------------------
        # 구조 변경 [박스 3]: [상세 목록] 영역 (필터, 검색, 삭제 에디터 통합)
        # --------------------------------------------------------------------------------
        with st.container():
            st.markdown('<div class="card-content-v2">', unsafe_allow_html=True)
            # 레이아웃 비율 및 구성 조정 (필터와 검색 정렬 처리)
